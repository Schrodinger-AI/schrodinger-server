...
public class PointAccumulateForSGR10Worker :  AsyncPeriodicBackgroundWorkerBase
{
    ...
    public PointAccumulateForSGR10Worker(AbpAsyncTimer timer,IServiceScopeFactory serviceScopeFactory,ILogger<PointAccumulateForSGR10Worker> logger,
        ...
        INESTRepository<AwakenLiquiditySnapshotIndex, string> pointSnapshotIndexRepository, 
    ...
    protected override async Task DoWorkAsync(PeriodicBackgroundWorkerContext workerContext)
    {
        ...
        if (!bizDateList.IsNullOrEmpty())
        {
            ...
            {
                await SGR10SnapshotForOnceAsync(bizDate, pointName);
            }
        }
        else
        {
            var bizDate = DateTime.UtcNow.ToString(TimeHelper.Pattern);
            await SGR10SnapshotWorkAsync(bizDate, pointName);
        }
    }
    
    
    private async Task SGR10SnapshotWorkAsync(string bizDate, string pointName)
    {
    ...
    private async Task SGR10SnapshotForOnceAsync(string bizDate, string pointName)
    {
    ...
    private async Task GenerateSnapshotAsync(string chainId, string bizDate, string pointName, int snapshotIndex, bool isCurrent)
    {
        ...
        _logger.LogInformation("PointAccumulateForSGR10Worker GetLiquidityRecordsAsync request: {result}, result: {record}", 
        ...
        _logger.LogInformation("PointAccumulateForSGR10Worker  liquidity address record counts: {cnt}", snapshots.Count);

        var validSnapshots = snapshots.Where(x => x.Token0Amount >= 0 || x.Token1Amount >= 0).ToList();
        
        _logger.LogInformation("PointAccumulateForSGR10Worker  valid record counts: {cnt}", validSnapshots.Count);
        ...
        if (snapshotIndex == SnapShotCount-1)
        {
            ...
            _logger.LogInformation("PointAccumulateForSGR10Worker  get valid address counts: {cnt}", validAddress.Count());
            
            var elfPrice = await GetELFPrice();
            var sgrPrice = await GetSGRPrice() * elfPrice;
            
            _logger.LogInformation("PointAccumulateForSGR10Worker  get prices in USD, ELF: {elf}, SGR: {sgr}", elfPrice, sgrPrice);
            ...
            foreach (var snapshot in snapshotByAddress)
            {
                ...
                {
                    _logger.LogInformation("PointAccumulateForSGR10Worker Holding Less Than 24hours, address: {address}", snapshot.Address);
                    continue;
                }
                ...
                if (pointAmount <= 0)
                {
                    _logger.LogInformation("PointAccumulateForSGR10Worker Liquidity Has No Value, address: {address}", snapshot.Address);
                    continue;
                }
    ...
    private async  Task<List<string>> GetValidAddressAsync(string chainId, string bizDate)
    {
        ...
        _logger.LogInformation("PointAccumulateForSGR10Worker get time in targetUtc: {targetUtc} targetUtcMidnight: {targetUtcMidnight}", targetUtc, targetUtcMidnight);
        ...
        _logger.LogInformation("PointAccumulateForSGR10Worker get snapshot in ts: {ts} counts: {cnt}", milliseconds, snapshots.Count);

        var validSnapshots = snapshots.Where(x => x.Token0Amount >= 0 || x.Token1Amount >= 0).ToList();
        _logger.LogInformation("PointAccumulateForSGR10Worker get valid snapshot in ts: {ts} counts: {cnt}", milliseconds, snapshots.Count);
